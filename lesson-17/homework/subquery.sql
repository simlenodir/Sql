...
-- task 3
truncate table #TempPurchases
create table #TempPurchases (puchaseId int identity, productName varchar(100), unitprice decimal(10, 2))
insert into #TempPurchases
SELECT 
	   [productname]
      ,[unitprice]
	  FROM [TSQL2012].[Production].[Products] as p
	  where p.productname not in (select productname from #TempPurchases)
...
-- task 5 
;create function fnSquare (@a int, @b int)
